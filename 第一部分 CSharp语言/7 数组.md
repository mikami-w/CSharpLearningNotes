...
​	`IStructuralEquatable`接口用于比较两个集合化对象(数组, 元组等)是否有相同的内容, `IStructuralComparable`接口用于集合化对象的结构化比较. 这两个接口不仅可以比较引用, 还可以比较内容. 这些接口是显示实现的, 所以在使用时需要把对象强制转换为这个接口类型. 数组实现了这两个接口.
